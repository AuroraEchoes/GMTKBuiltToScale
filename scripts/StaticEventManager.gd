...
signal door_hack_setup(door_id: int)
signal say_door_unlocked()
signal audio_collision(material: Global.MaterialType, velocity: float, relative_scl: float, position: Vector2)
signal audio_scale_effect(position: Vector2, up: bool)
